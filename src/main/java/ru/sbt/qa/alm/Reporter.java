...
/**
 * Created by sbt-neradovskiy-kl on 20.07.2015. version 1.0 Test case data
 * storage class
 */
class AlmTestCase {
...
/**
 * Created by sbt-neradovskiy-kl on 20.07.2015. version 1.0 ALM Reporter class
 */
public class Reporter {
    ...
    /**
     * Reads behavior.json from allure report results and fills collection of
     * the test cases that are going to be loaded to the ALM. Only tests that
     * start with a number are added to the collection
     *
     * @param reportFolder
     * @return
     */
    ...
    public void generateAlmTestRuns(String repBaseURL) {
        try {
            ...
            String owner = almprops.getProperty("alm.user"); //System.getProperty("user.name");
            ...
            for (AlmTestCase tc : almtcs) {
                ...
                String[] storyTitleFormated = tc.story.getTitle().split(" \\| ");
                
                logger.info("Publishing testcase " + storyTitleFormated[0] + " finished with status: " + status);
                ...
                String runname = storyTitleFormated[0].substring(tc.testId.length() + 1); //return Title without testid
                ...
                Long durationInSec;
                durationInSec = (durationInSec = duration / 1000) > 0 ? durationInSec : 1;
